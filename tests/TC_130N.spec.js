...
test.describe('TC_130N.spec', async () => {
    logAndTimeStamp('TC_130N.spec')
    test.setTimeout(600000)
    test.slow()

    test('GOMO', async () => {
        const commonTool = new CommonTool('GOMO-TC130N')
        ...
        await test.step('Voice Problem Page (New Tab)', async () => {
            logAndTimeStamp('Voice Problem Page')
            ...
            const voice = new VoicePage(newTab);
            await commonTool.takeScreenshot(newTab, 'GOMO', '6-ProblemPage');
            await voice.clickButtonVerify();
            await voice.selectProblemType('สัญญาณอ่อน/ไม่มีสัญญาณ/โทรออก-รับสายยาก');
            await page.waitForTimeout(3000);
            await voice.selectDate('10');
            await voice.selectTime('05', '02');
            await page.waitForTimeout(3000);
            await voice.clickButtonConfirm();
            await page.waitForTimeout(3000);
            await voice.clickButtonReport();
            await page.waitForTimeout(3000);
    })
})
})