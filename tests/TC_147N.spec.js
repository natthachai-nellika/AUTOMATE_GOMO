...
test.describe('TC_147N.spec', async () => {
    logAndTimeStamp('TC_147N.spec')
    test.setTimeout(600000)
    test.slow()

    test('GOMO', async () => {
        const commonTool = new CommonTool('GOMO-TC147N')
        ...
        await test.step('NET Problem Page (New Tab)', async () => {
            ...
            // แก้ไขจุดที่ 1: เติมเว้นวรรคระหว่าง 1 กับ App ให้ตรงกับหน้าจอจริง
            await net.selectProblemType('อินเทอร์เน็ตใช้งานไม่ได้ 1 App/Web');
            
            await newTab.waitForTimeout(3000);
            await net.selectDate('10');
            await net.selectTime('01', '02');
            await newTab.waitForTimeout(3000);
            await net.clickButtonConfirm();
            await commonTool.takeScreenshot(newTab, 'GOMO', '7-ProblemPage');
            await newTab.waitForTimeout(5000);

            // แก้ไขจุดที่ 2: เปลี่ยน netPage เป็น net ให้ตรงกับตัวแปรที่ประกาศไว้
            const appTab = await net.clickLinkApp();
            await commonTool.takeScreenshot(appTab, 'GOMO', '8-GomoAppPage');
